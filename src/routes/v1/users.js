...
router.post("/friends/new", verifyToken, async (req, res) => {
    ...
    if (!username || !discriminator) return res.status(400).json({ message: "Please provide a recipient." });

    const recipientRaw = await userModel.findOne({ username, discriminator });
    if (!recipientRaw) return res.status(404).json({ message: "The provided user could not be found." });

    const userHasFriend = req.user.friends.find((friend) => friend.username === otherUser.username);
    const otherUserHasFriend = recipientRaw.friends.find((friend) => friend.username === user.username);
    ...
    if (userHasFriend || otherUserHasFriend) {
     ...
     events.emit("FriendRequestCreate", { sender: req.user, recipient: recipientRaw });
     return res.status(200).json({ message: `Friend request sent to ${recipientRaw.username} successfully.` });
