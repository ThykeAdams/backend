...
router.post('/login', async (req, res) => {
  ...
  if (!user)
    res
      .status(404)
      .json({ message: 'A user with this username could not be found.' });
...
router.post('/register', async (req, res) => {
  ...
  if (!username || !email || !password || !discriminator || !dob) {
    return res
      ...
      .json({
        message:
          'Username, Email, Password, Discriminator, and Date of Birth (DOB) are required.',
      });
  }
  ...
  if (await userModel.findOne({ email }))
    return res
      .status(409)
      .json({ message: 'There is already a account with this Email.' });
  if (await userModel.findOne({ username, discriminator }))
    return res
      .status(409)
      .json({
        message:
          'There is already a account with this username and discriminator.',
      });
